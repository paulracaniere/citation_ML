...
IDs = [element[0] for element in node_info[1:]]

# compute TFIDF vector of each paper
corpus = [element[5] for element in node_info[1:]]
...
'''
five_percent = int(round(len(training_set)*0.05))
print(five_percent, "examples will be trained")
to_keep = random.sample(range(len(training_set)), k=five_percent)
...
with open("processed_train.csv", "w") as f:
    ...
    csv_write.writerow(["overlap_title","temp_diff","comm_auth","label"])
    for a,b in zip(training_features,labels_array):
        csv_write.writerow(list(a)+[str(b)])
...
with open("processed_test.csv", "w") as f:
    csv_write = csv.writer(f)
    csv_write.writerow(["overlap_title","temp_diff","comm_auth"])
    for a in testing_features:
        csv_write.writerow(list(a))
'''